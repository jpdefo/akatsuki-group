...
- **Steam packages (/sub/ giveaways)**: `resolve_package_app_ids` (server, runs in `--hydrate-sync-media`) classifies them from `steam-package-cache.json`: an edition (1 game + DLC) gets `appId` = the base game; a multi-game collection keeps `appId` = the sub id and lists its games in `packageAppIds` — HLTB is summed over the games' store names and the progress refresh sums achievements/playtime across them into one entry keyed by the sub id. Win records mirror their giveaway's `appId`.
