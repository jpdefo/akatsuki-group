...
- **Steam packages (/sub/ giveaways)**: `resolve_package_app_ids` (server, runs in `--hydrate-sync-media`) classifies them from `steam-package-cache.json`: an edition (1 game + DLC) gets `appId` = the base game; a multi-game collection keeps `appId` = the sub id and lists its games in `packageAppIds` — HLTB is summed over the games' `appId`s and the progress refresh sums achievements/playtime across them into one entry keyed by the sub id. Win records mirror their giveaway's `appId`.
- **HLTB lookups** (`lookup_hltb_hours`, `enrich_sync_with_hltb`) go through Augmented Steam's public backend (`GET https://api.augmentedsteam.com/app/<appId>/v2`, unauthenticated — the same one its browser extension calls), keyed by Steam `appId`, not title. This is a third-party, undocumented API with no published ToS/rate limits for external use — keep call volume comparable to what a normal refresh needs and don't reintroduce title-based fuzzy search against howlongtobeat.com directly.
