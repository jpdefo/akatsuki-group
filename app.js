  ...
  const items = debts
    ...
    .map((debt) => {
      ...
      return `<li>
        ...
        ${statusMarkup}
        ${buildPenaltyPeerComparison(debt)}
      </li>`;
    })
    .join("");
...
// How the rest of the group did on the same game, so an off-looking penalty can
// be checked against peers: if everyone who had the full window still fell short,
// the required hours/achievements are probably miscalibrated (too high).
//
// The median is computed only over "settled" winners — those who met the
// threshold or whose deadline has already passed. Winners still inside their
// grace window (may keep playing) are shown in the expandable list but excluded
// from the median so they don't drag it down.
function buildPenaltyPeerComparison(debt) {
  const gameId = debt.win.gameId;
  if (!gameId) {
    return "";
  }
  const reference = evaluateMonthlyProgress(debt.win);
  const requiredHours = Number(reference.requiredHours || 0);
  const requiredAchievements = Number(reference.requiredAchievements || 0);
  const totalAchievements = getGameAchievementsTotal(debt.game || findById("games", gameId));

  const peers = state.wins
    .filter((win) => win.gameId === gameId && win.id !== debt.win.id)
    .map((win) => {
      const member = findById("members", win.memberId);
      const info = getWinPenaltyInfo(win);
      const met = evaluateMonthlyProgress(win).badge !== "danger";
      // "Settled" = reached the bar or the grace window has closed. The only
      // status that is not settled is "coming-due" (unmet, still in grace).
      const settled = info ? info.status !== "coming-due" : met;
      return {
        name: String(member?.name || "Unknown member"),
        hours: Number(win.currentHours || 0),
        achievements: Number(win.earnedAchievements || 0),
        met,
        settled,
        tag: describePeerPenaltyStatus(info, met),
      };
    })
  ...
  const settledPeers = peers.filter((peer) => peer.settled);
  const metCount = settledPeers.filter((peer) => peer.met).length;
  // Several finished winners and none reached the bar -> the bar looks too high.
  const barTooHigh = settledPeers.length >= 2 && metCount === 0;

  const summaryBits = [`${peers.length} other winner${peers.length === 1 ? "" : "s"}`];
  if (settledPeers.length) {
    const medianHours = median(settledPeers.map((peer) => peer.hours));
    const medianAchievements = median(settledPeers.map((peer) => peer.achievements));
    const hoursText =
      requiredHours > 0
        ? `median ${formatHours(medianHours)} vs ${formatHours(requiredHours)} needed`
        : `median ${formatHours(medianHours)} played`;
    const achText =
      requiredAchievements > 0
        ? `median ${formatMedianCount(medianAchievements)}/${requiredAchievements} achs`
        : `median ${formatMedianCount(medianAchievements)} achs`;
    summaryBits.push(`${settledPeers.length} settled: ${hoursText} · ${achText}`);
  } else {
    summaryBits.push("no settled winners yet");
  }

  const warning = barTooHigh
    ? `<span class="penalty-peer-warning">⚠ none met it with the full window, so the parameters may be too high</span>`
    : "";

  const rows = peers
    .map((peer) => {
      const achCell = totalAchievements ? `${peer.achievements}/${totalAchievements}` : String(peer.achievements);
      return `
        <tr class="${peer.settled ? "" : "peer-pending"}">
          <td>${escapeHtml(peer.name)}</td>
          <td>${escapeHtml(formatHours(peer.hours))}</td>
          <td>${escapeHtml(achCell)}</td>
          <td>${escapeHtml(peer.tag)}</td>
        </tr>
      `;
    })
    .join("");
  ...
  return `
    <span class="penalty-peer-summary">Group: ${escapeHtml(summaryBits.join(" · "))}</span>
    ${warning}
    <details class="penalty-peers">
      <summary>Show ${peers.length} winner${peers.length === 1 ? "" : "s"}</summary>
      <table class="penalty-peer-table">
        <thead><tr><th>Winner</th><th>Played</th><th>Achs</th><th>Status</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </details>
  `;
...
// Short label for why a peer's outcome is (or isn't) counted in the median.
function describePeerPenaltyStatus(info, met) {
  if (!info) {
    return met ? "met" : "-";
  }
  switch (info.status) {
...
// Median of a numeric list (0 for empty). Even-length lists average the middle two.
...
// Achievement medians can land on .5; show whole counts plainly and halves as-is.
